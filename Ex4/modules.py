...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # z = X @ self.weights
        # return - np.sum(y * z - np.log(1 + np.exp(z))) / X.shape[0]
        # Calculate the linear transformation
        z = X @ self.weights

        # Compute the logistic loss
        logistic_loss = -np.sum(y * z - np.log(1 + np.exp(z))) / X.shape[0]

        # Normalize by the number of samples
        average_loss = logistic_loss / X.shape[0]
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # Calculate the exponentiated scores
        ez = np.exp(X @ self.weights)

        # Compute the gradient of the logistic loss
        gradient = -np.sum((y - (ez / (1 + ez)))[:, None] * X, axis=0) / X.shape[0]
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        # fidelity_output = self.fidelity_module_.compute_output(weights=self.weights, **kwargs)
        # regularization_output = self.regularization_module_.compute_output(weights=self.weights, **kwargs)
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        # fidelity_jacobian = self.fidelity_module_.compute_jacobian(weights=self.weights, **kwargs)
        # regularization_jacobian = self.regularization_module_.compute_jacobian(weights=self.weights, **kwargs)
