...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "none":
            ...
            module = RegularizedModule(LogisticModule(self.coefs_), L1(), self.lam_,
                                       include_intercept=self.include_intercept_, weights=init)
        elif self.penalty_ == "l2":
            module = RegularizedModule(LogisticModule(self.coefs_), L2(), self.lam_,
                                       include_intercept=self.include_intercept_, weights=init)
